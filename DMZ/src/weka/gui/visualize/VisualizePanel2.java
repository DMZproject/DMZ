 ...
 */
public class VisualizePanel2 extends PrintablePanel {
          ...
          if (e.getActionCommand().equals("Submit")) {
            if (m_splitListener != null && m_shapes != null) {
              ...
              try {
                ...
                VisualizePanel2.this.addPlot(newPlot);
              } catch (Exception ex) {
              ...
              try {
                VisualizePanel2.this.setXIndex(x);
                VisualizePanel2.this.setYIndex(y);
              } catch (Exception er) {
            ...
            m_plot2D.removeAllPlots();
            try {
              VisualizePanel2.this.addPlot(m_originalPlot);
            } catch (Exception ex) {
              System.err.println(ex);
              ex.printStackTrace();
            }
    ...
    public void addPlot(PlotData2D newPlot) throws Exception {
      if (m_plot2D.getPlots().size() == 0) {
        ...
        if (newPlot.m_useCustomColour && m_showClassPanel) {
          VisualizePanel2.this.remove(m_classSurround);
          switchToLegend();
          m_legendPanel.setPlotList(m_plot2D.getPlots());
        ...
        if (!newPlot.m_useCustomColour && m_showClassPanel) {
          VisualizePanel2.this.add(m_classSurround, BorderLayout.SOUTH);
  ...
  public JComboBox m_ColourCombo = new JComboBox();
  ...
  //edit
  protected JButton m_allBut=new JButton("전체보기");
  ...
  protected JLabel m_JitterLab = new JLabel("지터(Jitter)", SwingConstants.RIGHT);
  ...
  /**
   * This constructor allows a VisualizePanel2Listener to be set.
   * 
   * @param ls the listener to use
   */
  public VisualizePanel2(VisualizePanelListener ls) {
    this();
    m_splitListener = ls;
  }

  /**
   * Set the properties for the VisualizePanel2
   * 
  ...
  public VisualizePanel2() {
    ...
    combos.setBackground(Color.WHITE);
    combos.setLayout(gb);
    constraints.gridx = 0;
    constraints.gridy = 0;
 ...
 //   combos.add(mbts, constraints);
    ...
    m_Jitter.setBackground(Color.WHITE);
    ...
    constraints.gridwidth = 2;
    constraints.gridheight = 1;
    combos.add(m_allBut, constraints);
    
    m_classSurround = new JPanel();
    m_classSurround.setBorder(BorderFactory.createTitledBorder("클래스 색상"));
    m_classSurround.setBackground(Color.WHITE);
    m_classSurround.setLayout(new BorderLayout());

    m_classPanel.setBorder(BorderFactory.createEmptyBorder(15, 10, 10, 10));
    m_classPanel.setBackground(Color.WHITE);
   ...
   */
  public void allVisualize(){
	  final javax.swing.JFrame jf =
			  new javax.swing.JFrame("DMZ: Visualize");
	  ...
	  jf.setSize(800, 600);
	  jf.setVisible(true);
	  mp.setInstances(this.getInstances());
  }
  protected void openVisibleInstances(Instances insts) throws Exception {
    ...
    while (parent != null) {
      if (parent instanceof JFrame) {
        ((JFrame) parent).setTitle("DMZ Classifier Visualize: "
  ...
  public void setUpComboBoxes(Instances inst) {
    ...
    for (int i = 0; i < XNames.length; i++) {
      ...
      CNames[i] = "비교속성: " + inst.attribute(i).name() + type;
      if (m_preferredXDimension != null) {
    ...
    m_plotSurround.setBackground(Color.WHITE);
    m_plotSurround.setBorder((BorderFactory.createTitledBorder("데이터: "
      + inst.relationName())));
    try {
      if (prefX != -1) {
        setXIndex(prefX);
  ...
  public static void main(String[] args) {
    try {
      if (args.length < 1) {
        System.err.println("Usage : weka.gui.visualize.VisualizePanel2 "
      ...
      jf.setSize(500, 400);
      jf.getContentPane().setLayout(new BorderLayout());
      final VisualizePanel2 sp = new VisualizePanel2();
