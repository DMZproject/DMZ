...
public class HierarchicalClusterer extends AbstractClusterer implements
  ...
  public static final Tag[] TAGS_LINK_TYPE = { new Tag(SINGLE, "싱글"),
    new Tag(COMPLETE, "컴플리트"), new Tag(AVERAGE, "평균(AVERAGE)"),
    new Tag(MEAN, "평균(mean)"), new Tag(CENTROID, "도심(CENTROID)"),
    new Tag(WARD, "워드"), new Tag(ADJCOMPLETE, "ADJ컴플리트"),
    new Tag(NEIGHBOR_JOINING, "이웃연결") };
...
//      + "get the latter interpretation.";
	    return "false로 적용하면, 클러스터 사이의 거리는 설명된다. ";
  }
...
//      + " is not required";
	    return "True로 설정하면 클러스터가 Newick형식으로 출력할지를 나타낸다. ";
  }
...
//      + "depending on the Link type).";
	    return "두개의 개별적인 인스턴스 사이의 거리를 측정하기 위해 거리행렬을 적용";
  }
...
//      + " use neighbor joining algorithm.";
	    return "두개의 클러스터 사이의 거리를 측정하기위해 사용되는 방법이다.\n"
	      + "싱글:\n"
	      + " 싱글 링크 거리를 찾는다. 최소 링크로 알려져있는데 최근접 거리이다.\n"
	      + "컴플리트:\n"
	      + " 최대링크로 알려져있는 컴플리트 링크거리를 찾는데, 가장 최장의 거리이다.\n"
	      + "ADJ컴플리트:\n"
	      + " 컴플리트의 일종이지만 adjustment로서 최장의 거리 클러스터이다.\n"
	      + "평균(average):\n"
	      + " 두개의 클러스터 요소사이의 거리 평균이다.\n"
	      + "평균(mean): \n"
	      + " 클러스터의 병합한 거리의 평균값이다.\n"
	      + "도심(Centroid):\n"
	      + " 클러스터의 도심의 거리를 찾는다."
	      + "워드(ward):\n"
	      + " 클러스터를 병합에 의한 발견된 변화의 거리.\n"
	      + "이웃 연결\n"
	      + "이웃연결 알고리즘을 사용.\n ";
  }
  ...
  public String globalInfo() {
    return "병합적인 계보적 군집화(Agglomerative hierarchical clustering)\n";
  }
