...
public class AddID extends Filter implements UnsupervisedFilter, OptionHandler {
  ...
  public String globalInfo() {
    return "데이터에 존재하는 각 속성에 대해 고유 ID를 담고 있는 속성을 추가한다.  "
      + "식별자 속성은 다른 필터에 의해 변환되거나 인스턴스의 순서를 랜덤화시키는 등"
      + "다양한 방식으로 데이터를 처리한 이후에 개별 인스턴스들을 추정할 때 유용하게 쓸 수 있다. ";
  }
  ...
  public String IDIndexTipText() {
   ...
   return "속성 위치(시작 값 '1')에 삽입 되어집니다. (first 와 last값이 유효합니다.)";
  }
