...
public class NaiveBayes extends AbstractClassifier implements OptionHandler,
  ...
  public String globalInfo() {
    return "표준 확률 단순 베이지안 분류기. 커널 밀도 추정기를 이용할 수 있는데, 이는 정규성 가정이 전혀 맞지 않을 경우 성능을 향상시킨다. 또한 감독 이산화를 이용해 수치 속성들을 다룰 수 있다. \n";
  }
  ...
  public String useKernelEstimatorTipText() {
    return "수치 속성이 아닌 정규 분포에 대한 커널 추정기를 사용한다.";
  }
  ...
  public String useSupervisedDiscretizationTipText() {
    return "명목 데이터에 숫자 속성을 변환하는 감독 이산화를 사용합니다.";
  }
  ...
  public String displayModelInOldFormatTipText() {
    return "이전 형식을 사용하여 모델을 출력한다. 많은 클래스 값들이 존재하는 경우 이전 형식이 낫다."
    		+ "적은 수의 클래스와 많은 속성이 있을 때 새로운 형식이 더 좋다.";
  }
